...
var _ = ginkgo.Describe("Topology Aware Hints", func() {
	...
	ginkgo.It("Services with topology annotation should forward traffic to nodes on the same zone", func() {
		...
		// select one node and create a pod to generate the traffic
		...
		cmd := fmt.Sprintf("for i in $(seq 0 %d); do echo; %s ; sleep 0.1 ; done", maxConnections, nc)
		...
		framework.Logf("Connections from %s distributed with topology hints enabled %v Failed: %d", nodeZone, zones, failed)
